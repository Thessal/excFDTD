...
void RFT(void) {
	...
	for (int i = 0; i < _SURF_SIZE_; i++) {
		for (int j = 0; j < FREQ_N; j++) {
			...
			//Yee cell mismatch consideration //FIXME
			//imag
...
void NTFF(void) {
	...
	float k_vector;
	float *NF_eyePos, *xyz_to_sph,  *FF_ecSr_magnitude;
	MKL_Complex8 *Hankel_dx, *NF_ecE, *NF_H, *NF_ecM, *NF_J, *normal, 
		*NF_ecL_dx, *NF_N_dx, //free me
		*NF_ecL_sph_dx_, *NF_N_sph_dx_, *NF_ecM_temp, *NF_J_temp,
	...
	MKL_Complex16 *NF_ecL_sum_sph, *NF_N_sum_sph;//[0]:r [1]:theta [2]:phi
	...
	//rho_inv_dx = (float*)mkl_malloc(_SURF_SIZE_ * sizeof(float), 64);
	...
	NF_ecL_sph_dx_ = (MKL_Complex8*)mkl_malloc(3 * _SURF_SIZE_ * sizeof(MKL_Complex8), 64);
	NF_N_sph_dx_ = (MKL_Complex8*)mkl_malloc(3 * _SURF_SIZE_ * sizeof(MKL_Complex8), 64);
	...
	FF_H_theta = (MKL_Complex8*)mkl_malloc(NTFF_IMG_SIZE*NTFF_IMG_SIZE * sizeof(MKL_Complex8), 64);
	FF_H_phi = (MKL_Complex8*)mkl_malloc(NTFF_IMG_SIZE*NTFF_IMG_SIZE * sizeof(MKL_Complex8), 64);
	...
	NF_ecL_sum_sph = (MKL_Complex16*)mkl_malloc(3 * NTFF_IMG_SIZE*NTFF_IMG_SIZE * sizeof(MKL_Complex16), 64);
	NF_N_sum_sph = (MKL_Complex16*)mkl_malloc(3 * NTFF_IMG_SIZE*NTFF_IMG_SIZE * sizeof(MKL_Complex16), 64);
	NF_ecL_sum_temp = (MKL_Complex16*)mkl_malloc(NTFF_IMG_SIZE*NTFF_IMG_SIZE * sizeof(MKL_Complex16), 64);
	NF_N_sum_temp = (MKL_Complex16*)mkl_malloc(NTFF_IMG_SIZE*NTFF_IMG_SIZE * sizeof(MKL_Complex16), 64);
	FF_ecSr1 = (MKL_Complex8*)mkl_malloc(NTFF_IMG_SIZE*NTFF_IMG_SIZE * sizeof(MKL_Complex8), 64);
	...
	for (int freqN = 0; freqN < FREQ_N; freqN++) { //each frequency
		...
		for (int k = 0; k < _SURF_SIZE_; k++) {
			NF_ecE[0 * _SURF_SIZE_ + k].real = FT_eps0cE[k][freqN][0][0];
			NF_ecE[1 * _SURF_SIZE_ + k].real = FT_eps0cE[k][freqN][1][0];
			NF_ecE[2 * _SURF_SIZE_ + k].real = FT_eps0cE[k][freqN][2][0];
			NF_ecE[0 * _SURF_SIZE_ + k].imag = FT_eps0cE[k][freqN][0][1];
			NF_ecE[1 * _SURF_SIZE_ + k].imag = FT_eps0cE[k][freqN][1][1];
			NF_ecE[2 * _SURF_SIZE_ + k].imag = FT_eps0cE[k][freqN][2][1];
			NF_H[0 * _SURF_SIZE_ + k].real = FT_H[k][freqN][0][0];
			NF_H[1 * _SURF_SIZE_ + k].real = FT_H[k][freqN][1][0];
			NF_H[2 * _SURF_SIZE_ + k].real = FT_H[k][freqN][2][0];
		...
		printf("NTFF calculation\n");
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {
			printf("%f%%\r", 100.0f*(float)i / NTFF_IMG_SIZE);
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				float radius = ((float)NTFF_IMG_SIZE - 1.0f) / 2.0f;
				float xx = ((float)i - radius);
				float yy = ((float)j - radius);
				...
				ippsZero_64fc(NF_ecL_sum, 3);
				ippsZero_64fc(NF_N_sum, 3);
				...
				for (int ii = 0; ii < 3; ii++) { // R theta phi
					...
					//64
					//ippsSum_32fc(NF_ecL_sph_dx + ii * _SURF_SIZE_, _SURF_SIZE_, NF_ecL_sum + ii, IPP_FFT_DIV_BY_SQRTN); //FIXME : IPP_FFT_NODIV_BY_ANY?
					//ippsSum_32fc(NF_N_sph_dx + ii * _SURF_SIZE_, _SURF_SIZE_, NF_N_sum + ii, IPP_FFT_DIV_BY_SQRTN);
					//printf("%1.2e/%1.2e ",NF_ecL_sph_dx[ii * _SURF_SIZE_].real, NF_ecL_sph_dx[1+ii*_SURF_SIZE_].real);
		...
		ippsZero_64fc(NF_ecL_sum_sph, 3 * NTFF_IMG_SIZE*NTFF_IMG_SIZE);
		ippsZero_64fc(NF_N_sum_sph, 3 * NTFF_IMG_SIZE*NTFF_IMG_SIZE);
		...
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				...
				NF_ecL_sum_sph[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real =
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_ecL_sum_sph[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real = 
					-NF_ecL_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_ecL_sum_sph[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real = (
					-NF_ecL_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* 
						NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] 
					-NF_ecL_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* 
						NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					+NF_ecL_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* 
						NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					) / NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_N_sum_sph[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real =
					NF_N_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_N_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_N_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_N_sum_sph[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real =
					-NF_N_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_N_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_N_sum_sph[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real = (
					-NF_N_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					- NF_N_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					+ NF_N_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].real*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					) / NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];

				NF_ecL_sum_sph[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag =
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_ecL_sum_sph[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag =
					-NF_ecL_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_ecL_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_ecL_sum_sph[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag = (
					-NF_ecL_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					- NF_ecL_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					+ NF_ecL_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					) / NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_N_sum_sph[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag =
					NF_N_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_N_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_N_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_N_sum_sph[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag =
					-NF_N_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] +
					NF_N_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag* NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];
				NF_N_sum_sph[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag = (
					-NF_N_sum[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					- NF_N_sum[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					+ NF_N_sum[i + j*NTFF_IMG_SIZE + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE].imag*
					NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE] * NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE]
					) / NF_eyePos[i + j*NTFF_IMG_SIZE + 3 * NTFF_IMG_SIZE *NTFF_IMG_SIZE];

				FF_ecE_theta[i + j*NTFF_IMG_SIZE].real = NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real + NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;
				FF_ecE_phi[i + j*NTFF_IMG_SIZE].real = - NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real + NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;
				FF_H_theta[i + j*NTFF_IMG_SIZE].real = NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real - NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;
				FF_H_phi[i + j*NTFF_IMG_SIZE].real = - NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real - NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;

				FF_ecE_theta[i + j*NTFF_IMG_SIZE].imag = NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag + NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;
				FF_ecE_phi[i + j*NTFF_IMG_SIZE].imag = - NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag + NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;
				FF_H_theta[i + j*NTFF_IMG_SIZE].imag = NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag - NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;
				FF_H_phi[i + j*NTFF_IMG_SIZE].imag = - NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag - NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;

				//FF_ecE_theta[i + j*NTFF_IMG_SIZE].real = NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real - NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;
				//FF_ecE_phi[i + j*NTFF_IMG_SIZE].real = -NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real - NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;
				//FF_H_theta[i + j*NTFF_IMG_SIZE].real = - NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real - NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;
				//FF_H_phi[i + j*NTFF_IMG_SIZE].real =  NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real - NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].real;

				//FF_ecE_theta[i + j*NTFF_IMG_SIZE].imag = NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag - NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;
				//FF_ecE_phi[i + j*NTFF_IMG_SIZE].imag = -NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag - NF_ecL_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;
				//FF_H_theta[i + j*NTFF_IMG_SIZE].imag = - NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag - NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;
				//FF_H_phi[i + j*NTFF_IMG_SIZE].imag = NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag - NF_N_sum_sph[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i + j*NTFF_IMG_SIZE].imag;
		...
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//ippsAdd_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//ippsAdd_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//ippsAdd_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//Vtheta = [Vx,Vy,Vz] . [-ry,rx,0]
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//ippsSub_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//ippsAdd_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		////Vphi = [Vx,Vy,Vz] . [-rx*rz,-ry*rz,+rr*rr] / rr
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum_temp, NF_eyePos + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp); //warning : in-place op
		//ippsSub_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum_temp, NF_eyePos + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//ippsSub_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 4 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_ecL_sum_temp, NF_eyePos + 4 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_temp);
		//ippsAdd_64fc_I(NF_ecL_sum_temp, NF_ecL_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//ippsDiv_64fc_I(NF_eyePos + 4 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);

		////Vr = [Vx,Vy,Vz] . [rx,ry,rz]
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//ippsAdd_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//ippsAdd_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//ippsAdd_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		////Vtheta = [Vx,Vy,Vz] . [-ry,rx,0]
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//ippsSub_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//ippsAdd_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		////Vphi = [Vx,Vy,Vz] . [-rx*rz,-ry*rz,+rr*rr] / rr
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum_temp, NF_eyePos + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp); //warning : in-place op
		//ippsSub_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum_temp, NF_eyePos + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//ippsSub_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_eyePos + 4 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//vzMul((const int)(NTFF_IMG_SIZE *NTFF_IMG_SIZE), NF_N_sum_temp, NF_eyePos + 4 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_temp);
		//ippsAdd_64fc_I(NF_N_sum_temp, NF_N_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//ippsDiv_64fc_I(NF_eyePos + 4 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NTFF_IMG_SIZE *NTFF_IMG_SIZE);

		//ch14, (14.54)  (double check signs of M and J!)
		////[0]:r [1]:theta [2]:phi
		//64fc?
		//ippsAdd_32fc(NF_N_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, FF_ecE_theta, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//ippsSub_32fc(NF_ecL_sum + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_sph + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, FF_ecE_phi, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//ippsSub_32fc(NF_N_sum + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_ecL_sum_sph + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, FF_H_theta, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		//ippsAdd_32fc(NF_ecL_sum + 2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, NF_N_sum_sph + 1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE, FF_H_phi, NTFF_IMG_SIZE *NTFF_IMG_SIZE);
		...
		ippsMulC_32fc_I((Ipp32fc) { sqrtf(0.125f / M_PI * k_vector), 0.0f }, FF_H_phi, NTFF_IMG_SIZE*NTFF_IMG_SIZE);
		...
		for (int i = 0; i < NTFF_IMG_SIZE; i++)	{		for (int j = 0; j < NTFF_IMG_SIZE; j++) {
			//float val = NF_eyePos[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i] * 255;
			//float val = sqrtf(FF_ecE_theta[j*NTFF_IMG_SIZE + i].real *  FF_ecE_theta[j*NTFF_IMG_SIZE + i].real +  FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag *  FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 1000.0f;
			//float val = sqrtf(FF_ecE_phi[j*NTFF_IMG_SIZE + i].real * FF_ecE_phi[j*NTFF_IMG_SIZE + i].real + FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag * FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 2.0f;
			//float val = FF_H_phi[j*NTFF_IMG_SIZE + i].real * 255.0f * 10.0f;  
			float val = FF_ecSr[j*NTFF_IMG_SIZE + i].real * 255.0f / 300000.0f;
			//float val = FF_ecSr1[j*NTFF_IMG_SIZE + i].real * 255.0f * 500.0f * 500.0f;
			//float val = FF_ecSr2[j*NTFF_IMG_SIZE + i].real * 255.0f * 500.0f * 500.0f;
			//float val = FF_ecSr_magnitude[j*NTFF_IMG_SIZE + i] * 255.0f * 500.0f;
			//double val = FF_ecE_theta[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].real * 255.0 * 100.0;
			//double val =				(
			//	NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].real *
			//	NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].real+
			//	NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].imag*
			//	NF_ecL_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].imag
			//	)* 255.0 / 10.0;
			//double val = (
			//	NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].real *
			//	NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].real +
			//	NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].imag*
			//	NF_N_sum_sph[2 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].imag
			//	)* 255.0 / 10.0;
			//float val = FF_H_theta[j*NTFF_IMG_SIZE + i].real *255.0f /10000.0f;
			//float val = FF_ecE_theta[j*NTFF_IMG_SIZE + i].real * 255.0f / 10000.0f;
			//float val = sqrt(
			//	FF_ecE_phi[j*NTFF_IMG_SIZE + i].real*FF_ecE_phi[j*NTFF_IMG_SIZE + i].real
			//	+ FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag*FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag
			//	//+ FF_ecE_theta[j*NTFF_IMG_SIZE + i].real*FF_ecE_theta[j*NTFF_IMG_SIZE + i].real
			//	//+ FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag*FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag
			//) * 255.0f / 50.0f;
			//float val = sqrt(
			//	FF_ecE_phi[j*NTFF_IMG_SIZE + i].real*FF_ecE_phi[j*NTFF_IMG_SIZE + i].real
			//	//+ FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag*FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag
			...
			//	//+ FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag*FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag
			//) * 255.0f / 300.0f;
			//double val = NF_N_sum_sph[0 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + j*NTFF_IMG_SIZE + i].real * 255.0 * 100.0;
		...
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {		for (int j = 0; j < NTFF_IMG_SIZE; j++) {
			float val = sqrtf(FF_ecE_theta[j*NTFF_IMG_SIZE + i].real *  FF_ecE_theta[j*NTFF_IMG_SIZE + i].real + FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag *  FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f;
		...
		}}error = lodepng_encode32_file("E_theta.png", image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {		for (int j = 0; j < NTFF_IMG_SIZE; j++) {
			float val = sqrtf(FF_ecE_phi[j*NTFF_IMG_SIZE + i].real * FF_ecE_phi[j*NTFF_IMG_SIZE + i].real + FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag * FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f;
			image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = val > 0 ? (val < 255 ? val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = val < 0 ? (val > -255 ? -val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;  image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
		}}error = lodepng_encode32_file("E_phi.png", image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {		for (int j = 0; j < NTFF_IMG_SIZE; j++) {
			float val = 
				sqrtf(FF_ecE_theta[j*NTFF_IMG_SIZE + i].real *  FF_ecE_theta[j*NTFF_IMG_SIZE + i].real + FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag *  FF_ecE_theta[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f
			+sqrtf(FF_ecE_phi[j*NTFF_IMG_SIZE + i].real * FF_ecE_phi[j*NTFF_IMG_SIZE + i].real + FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag * FF_ecE_phi[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f;
			image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = val > 0 ? (val < 255 ? val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = val < 0 ? (val > -255 ? -val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;  image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
		}}error = lodepng_encode32_file("E.png", image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);


		for (int i = 0; i < NTFF_IMG_SIZE; i++) {
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				float val = sqrtf(FF_H_theta[j*NTFF_IMG_SIZE + i].real *  FF_H_theta[j*NTFF_IMG_SIZE + i].real + FF_H_theta[j*NTFF_IMG_SIZE + i].imag *  FF_H_theta[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f;
				image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = val > 0 ? (val < 255 ? val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = val < 0 ? (val > -255 ? -val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;  image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
			}
		}error = lodepng_encode32_file("H_theta.png", image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				float val = sqrtf(FF_H_phi[j*NTFF_IMG_SIZE + i].real * FF_H_phi[j*NTFF_IMG_SIZE + i].real + FF_H_phi[j*NTFF_IMG_SIZE + i].imag * FF_H_phi[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f;
				image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = val > 0 ? (val < 255 ? val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = val < 0 ? (val > -255 ? -val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;  image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
			}
		}error = lodepng_encode32_file("H_phi.png", image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				float val =
					sqrtf(FF_H_theta[j*NTFF_IMG_SIZE + i].real *  FF_H_theta[j*NTFF_IMG_SIZE + i].real + FF_H_theta[j*NTFF_IMG_SIZE + i].imag *  FF_H_theta[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f
					+ sqrtf(FF_H_phi[j*NTFF_IMG_SIZE + i].real * FF_H_phi[j*NTFF_IMG_SIZE + i].real + FF_H_phi[j*NTFF_IMG_SIZE + i].imag * FF_H_phi[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300.0f;
				image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = val > 0 ? (val < 255 ? val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = val < 0 ? (val > -255 ? -val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;  image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
			}
		}error = lodepng_encode32_file("H.png", image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);


		for (int i = 0; i < NTFF_IMG_SIZE; i++) {
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				float val = sqrtf(FF_ecSr[j*NTFF_IMG_SIZE + i].real * FF_ecSr[j*NTFF_IMG_SIZE + i].real + FF_ecSr[j*NTFF_IMG_SIZE + i].imag * FF_ecSr[j*NTFF_IMG_SIZE + i].imag) * 255.0f / 300000.0f;
				image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = val > 0 ? (val < 255 ? val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = val < 0 ? (val > -255 ? -val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;  image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
			}
		...
		for (int i = 0; i < NTFF_IMG_SIZE; i++) {
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				float val = FF_ecSr_magnitude[j*NTFF_IMG_SIZE + i] * 255.0f / 300000.0f;
				image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = val > 0 ? (val < 255 ? val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = val < 0 ? (val > -255 ? -val : 255) : 0;		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;  image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
			}
		}error = lodepng_encode32_file("S_r_mag.png", image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);
		...
		for (int j = 0; j < _SURF_DY_ ; j++)	{
			for (int i = 0; i < _SURF_DX_ ; i++) {
				////float val = rho_inv_dx[_SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)] *255.0f * 20.0f;
				//float val = normal[_SURF_SIZE_ * 1 + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].real *255.0f * 5.0f;
				//float val = normal[_SURF_SIZE_ * 1 + _SURF_INDEX_XYZ( _SURF_StartZ_,_SURF_StartX_+ i, _SURF_StartY_+j)].real *255.0f * 5.0f ;
				float val = NF_ecM[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].real *255.0f * 10000.0f;
...
//				float val = Hankel_dx[_SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].real *255.0f;
				//float val = Hankel_dx[_SURF_INDEX_XYZ(_SURF_StartZ_,_SURF_StartX_ + i, _SURF_StartY_ + j)].real *255.0f;
				//float val = sqrt(
				//	NF_J_sph[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].imag *
				//	NF_J_sph[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].imag+
				//	NF_J_sph[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].real *
				//	NF_J_sph[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].real
				//	)*255.0f * 500.0f;
				//float val =  NF_ecL_sph_dx[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].real*255.0f * 500.0f;
				...
				//float val = NF_ecL_sph_dx[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ( _SURF_StartZ_,_SURF_StartX_ + i, _SURF_StartY_ + j)].real *255.0f * 5000.0f;
				//float val = NF_ecL_sph_dx[(1 * _SURF_SIZE_) + _SURF_INDEX_XYZ(_SURF_StartX_ + i, _SURF_StartY_ + j, _SURF_StartZ_)].real *255.0f * 5000.0f;
		...
		//for (int i = 0; i < NTFF_IMG_SIZE; i++)
		//	for (int j = 0; j < NTFF_IMG_SIZE; j++) {
		//		image[4 * NTFF_IMG_SIZE * j + 4 * i + 0] = FF_phi[i][j] * 50;
		//		image[4 * NTFF_IMG_SIZE * j + 4 * i + 1] = 0;
		//		image[4 * NTFF_IMG_SIZE * j + 4 * i + 2] = 0;
		//		image[4 * NTFF_IMG_SIZE * j + 4 * i + 3] = 255;
		//	}
		//error = lodepng_encode32_file(filename, image, NTFF_IMG_SIZE, NTFF_IMG_SIZE);
		...
		for (int i = 0; i < NTFF_IMG_SIZE; i++){
			for (int j = 0; j < NTFF_IMG_SIZE; j++) {
				fprintf(f_up, "%+04.3e\t", NF_eyePos[1 * NTFF_IMG_SIZE *NTFF_IMG_SIZE + i*NTFF_IMG_SIZE + j] );
	...
	//mkl_free(rho_inv_dx);
	mkl_free(Hankel_dx);
	mkl_free(NF_ecE); mkl_free(NF_H); mkl_free(NF_ecM); mkl_free(NF_J); mkl_free(normal);
